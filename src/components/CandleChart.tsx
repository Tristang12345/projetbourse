/**
 * CandleChart — Graphique OHLCV interactif (recharts).
 * Clic sur le bouton Chart d'une position → graphique détaillé.
 * Périodes : 1W, 1M, 3M, 6M, 1Y.
 * La clé de cache inclut le nombre de jours → changement de période OK.
 */
  ...
  return (
    <div className="bg-terminal-elevated border border-terminal-border rounded-md p-3 text-2xs font-mono shadow-panel">
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-0.5">
        ...
        <span className={colorClass(d.change ?? 0)}>
...
export const CandleChart: React.FC<CandleChartProps> = ({ ticker, currency, name, onClose }) => {
  ...
  // La clé de cache dans getCandles inclut maintenant les jours →
  // chaque changement de période charge les bonnes données.
  useEffect(() => {
    setLoading(true);
    setCandles([]);
  ...
  const maxPrice = candles.length ? Math.max(...candles.map((c) => c.high)) : 0;
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-3 border-b border-terminal-border">
          ...
          <div className="flex items-center gap-1">
            ...
            <button onClick={onClose} className="ml-3 text-terminal-dim hover:text-terminal-text transition-colors p-1">
              <X size={14} />
            </button>
          </div>
        </div>
                ...
                <ComposedChart data={chartData} margin={{ top: 5, right: 10, bottom: 5, left: 10 }}>
                  ...
                  <Bar
                    ...
                    shape={(props: any) => {
                      ...
                      return (
                        <g>
                          <line x1={wickX} y1={bg.y ?? y - 5} x2={wickX} y2={y}
                            stroke={color} strokeWidth={1} opacity={0.6} />
                          <rect x={x + 1} y={y} width={Math.max(width - 2, 1)}
                            height={Math.max(height, 1)} fill={color} opacity={0.85} rx={1} />
                          <line x1={wickX} y1={y + height}
                            x2={wickX} y2={(bg.y ?? y) + (bg.height ?? 0)}
                            stroke={color} strokeWidth={1} opacity={0.6} />
                        </g>
                      );
                    }}
                  ...
                  {/* Ligne de clôture */}
                  <Line dataKey="close" dot={false} stroke="#1a9fff" strokeWidth={1} opacity={0.3} />

                  {/* Ligne de référence premier cours */}
                  {firstCandle && (
                    <ReferenceLine y={firstCandle.close} stroke="rgba(255,255,255,0.1)" strokeDasharray="4 4" />
                  )}
                </ComposedChart>
              </ResponsiveContainer>
              ...
              <div className="grid grid-cols-4 gap-3 mt-3 pt-3 border-t border-terminal-border">
                {[
                  ...
                  <div key={label} className="text-center">
                    ...
                    <div className={`text-xs font-mono font-semibold mt-0.5 ${color ?? "text-terminal-text"}`}>
                      {val}
                    </div>
                  </div>
                ))}
